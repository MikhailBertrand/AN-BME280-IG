...
class BME280:
    def __init__(self, VID=0x04D8, PID=0x00DD, devnum=0):
        self.bus = SMBus.SMBus(VID, PID, devnum)
    ...
    def get_calib_param(self):
        ...
        self.__dig_temp.append((calib[1] << 8) | calib[0])
        self.__dig_temp.append((calib[3] << 8) | calib[2])
        self.__dig_temp.append((calib[5] << 8) | calib[4])
        ...
        self.__dig_hum.append(calib[24])
        self.__dig_hum.append((calib[26]<< 8) | calib[25])
        self.__dig_hum.append(calib[27])
        self.__dig_hum.append((calib[28]<< 4) | (0x0F & calib[29]))
        self.__dig_hum.append((calib[30]<< 4) | ((calib[29] >> 4) & 0x0F))
        self.__dig_hum.append(calib[31])

        for i in range(1, 2):
            if self.__dig_temp[i] & 0x8000:
                self.__dig_temp[i] = (- self.__dig_temp[i] ^ 0xFFFF) + 1
        ...
        for i in range(0, 6):
            if self.__dig_hum[i] & 0x8000:
                self.__dig_hum[i] = (-self.__dig_hum[i] ^ 0xFFFF) + 1
    ...
    def read_data(self):
        ...
        self.temperature = self.compensate_temp(temp_raw)
        self.pressure = self.compensate_P(pres_raw)
        self.humidity = self.compensate_hum(hum_raw)
        self.var_h = self.compensate_hum(hum_raw)
    ...
    def compensate_P(self, adc_P):
        ...
        v1 = (((self.__dig_pressure[2] * (((v1 / 4.0) * (v1 / 4.0)) / 8192)) / 8)  + ((self.__dig_pressure[1] * v1) / 2.0)) / 262144
        ...
        v1 = (self.__dig_pressure[8] * (((pressure / 8.0) * (pressure / 8.0)) / 8192.0)) / 4096
    ...
    def compensate_temp(self, adc_T):
        global t_fine
        v1 = (adc_T / 16384.0 - self.__dig_temp[0] / 1024.0) * self.__dig_temp[1]
        v2 = (adc_T / 131072.0 - self.__dig_temp[0] / 8192.0) * (adc_T / 131072.0 - self.__dig_temp[0] / 8192.0) * self.__dig_temp[2]
        t_fine = v1 + v2
    ...
    def compensate_hum(self, adc_H):
        ...
        if var_h != 0:
            var_h = (adc_H - (self.__dig_hum[3] * 64.0 + self.__dig_hum[4]/16384.0 * var_h)) * (self.__dig_hum[1] / 65536.0 * (1.0 + self.__dig_hum[5] / 67108864.0 * var_h * (1.0 + self.__dig_hum[2] / 67108864.0 * var_h)))
        else:
            return 0
        var_h = var_h * (1.0 - self.__dig_hum[0] * var_h / 524288.0)
        if var_h > 100.0:
            var_h = 100.0
        elif var_h < 0.0:
            var_h = 0.0
    ...
    def setup(self):
        ...
        filtering = 0			# Filter off
        spi3w_en = 0			# 3-wire SPI Disable

        ctrl_meas_reg = (osrs_t << 5) | (osrs_p << 2) | mode
        config_reg = (t_sb << 5) | (filtering << 2) | spi3w_en
        ctrl_hum_reg = osrs_h
